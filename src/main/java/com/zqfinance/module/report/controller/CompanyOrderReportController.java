 ...
 */
@Controller
@RequestMapping("/companyOrderReport")
public class CompanyOrderReportController extends DwzAjaxController{
	
	@RequestMapping("/orderReportMonth")
	public String OrderReportMonth() {
		return "report/companyOrderRptMonth";
	}
	...
	public RptVO getOrderMoneyYear(String year) throws ParseException, IOException {
		...
		String resultString = sendRequestService.sendRequest(
				ConfigManager.COMPANYORDERRPT_GETORDERRPTAGE, jsonObject);
...
//			
		...
		rptVO.setTitle(year +"充值数据统计");
		rptVO.setSubTitle("本年充值金额:"+ FromatMoneyUtil.fromatMoney(Double.parseDouble(countMoney.toString())));
	...
	public void delYearExcel(HttpServletRequest request, HttpServletResponse response) throws com.alibaba.dubbo.common.json.ParseException {
		...
		ExcelUtil.modelOne(rptVO, response, "（"+rptVO.getYear()+"年   公司充值月订单资金总额对照表", "月份");
	}
