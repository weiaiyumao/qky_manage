...
function dwzPageBreak(options){
	...
	if (op.rel) {
		...
		op.data["numPerPage"] = options.data["numPerPage"];
 ...
 */
function uploadifyError(file, errorCode, erromanageg) {
	alertMsg.error(errorCode+": "+erromanageg);
}
...
$.fn.extend({
	...
	dwzExport: function(){
		function _doExport($this) {
			...
			if(string.indexOf("ids")!=-1){
				window.location = url+(url.indexOf('?') == -1 ? "?" : "&")+string;
			}else{
				if($this.attr("pagechecked") == "checked"){
					window.location = url+(url.indexOf('?') == -1 ? "?" : "&")+string;
				}else{
					alertMsg.confirm("请选择您要导出的数据",null);
				}
			}
