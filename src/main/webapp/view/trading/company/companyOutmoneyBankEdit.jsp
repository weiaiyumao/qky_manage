...
<div class="pageContent">
	...
	<form name="form1" method="post" action="${ctx}/company/updateCompanyOutmoneyBank.htm" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
		<div class="pageFormContent" layoutH="56">
		<h2 class="contentTitle">编辑公司银行卡信息详情</h2>
			<p>	
				<label>名称：</label>
				<input type="hidden" name="id" id="id" value="${dataMessage.data['id'] }" maxlength="20">
				<input type="text" name="acctName" id="acctName" value="${dataMessage.data['acctName'] }" maxlength="20">
			</p>
			<p>	
				<label>证件卡号：</label>
				<input type="text" name="idNo" id="idNo" value="${dataMessage.data['idNo'] }" maxlength="20" >
			</p>
			<p>	
				<label>身份证类型：</label>
			...
			<p>	
				<label>银行卡号：</label>
				<input type="text" name="cardNo" id="cardNo" onkeypress="keyPress()" value="${dataMessage.data['cardNo']}" maxlength="20" >
			</p>
			<p>	
				<label>银行卡类型：</label>
			...
			<p>	
				<label>所属银行编号：</label>
				<input type="text" name="bankCode" id="bankCode" value="${dataMessage.data['bankCode'] }" maxlength="20" >
			</p>
			<p>	
				<label>所属银行名称：</label>
				<input type="text" name="bankName" id="bankName" value="${dataMessage.data['bankName'] }" maxlength="20" >
			</p>
			<p>	
				<label>开户行编号：</label>
			...
			<p>	
				...
				<select class="required" id="provinceName"  name="provinceName" onchange="getCityName();">
					...
					<c:forEach var="province" items="${dataMessage_province.contentList }">
						<option value="${province.provinceName},${province.serialNumber}" <c:if test="${province.provinceName == dataMessage.data['provinceName']}">selected</c:if> >${province.provinceName}</option>
					</c:forEach>
				</select>	
			</p>
			<p>	
			...
			<p>	
				...
				<select id="cityName" class="required"  name="cityName" >
					<option value="${dataMessage.data['cityName']}" class="toSelected">${dataMessage.data['cityName']}</option>
				</select>
			</p>
		</div>
		<div class="formBar">
