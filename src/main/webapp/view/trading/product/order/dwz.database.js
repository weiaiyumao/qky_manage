		...
		multLookup: function(){
			return this.each(function(){
				...
				$this.click(function(event){
					...
					//清空 原来的列表
					$("#PageList").children(".pagelist").remove(".pagelist");
					$unitBox.find("[name='"+$this.attr("multLookup")+"']").filter(":checked").each(function(i){
						var _args = DWZ.jsonEval($(this).val());
						var pageList ='<div style="clear: both;float: right;margin-right:20px;" id="list'+i+'" class="pagelist"><a id="list'+i+'" href="javascript:removelistRow('+i+');" class="btnb delposition pagelist" style="margin-top:-6px;">取消该笔</a><a id="list'+i+'" href="javascript:removelistRow('+i+');" style="color:green;" class="pagelist">删掉该笔</a></div><div class="divider pagelist" id="list'+i+'"></div>';
						var parm ="";
						for (var key in _args) {
							var value = args[key] ? args[key]+"," : "";
							args[key] = value + _args[key];
							if(key.indexOf("id") > -1){
								pageList = pageList + '<p class="pagelist" id="list'+i+'"><label>贷款标编号：</label><input id="'+key+'" readonly="readonly" name="'+key+'" type="text" size="30" value="" class="textInput readonly valid"/></p>';
								parm = parm + key;
							}
							if(key.indexOf("surplusMoney") > -1){
								pageList = pageList + '<p class="pagelist" id="list'+i+'"><label>剩余可投金额：</label><input id="'+key+'" readonly="readonly" name="'+key+'" type="text" size="30" value="" class="textInput readonly valid"/></p>';
							}
						}
						pageList = pageList + '<p class="pagelist" id="list'+i+'"><label>投入金额：</label><input id="money'+parm+'" class="required number textInput" name="money'+parm+'" type="text" size="30" value="" maxlength="19" onblur="checkTenderMoney(&quot;money'+parm+'&quot;)" /></p>';
						
						$("#appendList").before(pageList);
					});
