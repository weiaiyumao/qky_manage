...
<title>用户银行卡信息管理</title>
<script type="text/javascript">
$(document).ready(function(){
	//控制每页显示条数
	var num = ${dataMessage.page.numPerPage};
	$("#customerLCList").val(num);
});
function navTabAjaxDone(json){
   	 DWZ.ajaxDone(json);
 	 if (json.statusCode == DWZ.statusCode.ok){
 		 //列表页面重新加载数据
          navTab.reloadFlag("customerLCmgr");    
...
<form id="pagerForm" rel="pagerForm" onsubmit="return navTabSearch(this);"  method="post" action="${ctx}/customerlc/getCustomerLCBankList.htm">
...
readonly="true" value="${endDate }"/>
				...
				<td>
					用户名称：<input type="text" name="acctName" id="acctName" maxlength="20" value="${acctName }" />
				</td>
				<td>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">      
			<li><mgr:button href="${ctx}/customerlc/forwardInfo.htm?id={id_customerLC}"  target="navTab" value="详情" title="详情" cssClass="edit"></mgr:button></li>
			<li><mgr:button href="${ctx}/customerlc/forwardEdit.htm?id={id_customerLC}" target="navTab" value="修改" warn="请选择需要修改的银行卡信息" cssClass="edit"></mgr:button></li>
<%-- 			<li><mgr:button href="${ctx}/customerlc/delCustomerLCBank.htm" title="确实要删除这些银行卡记录吗?" target="selectedTodo" postType="string" value="删除" cssClass="delete"></mgr:button> --%>
		</ul>
	</div>
	...
	<div class="panelBar">
		<div class="pages">
			<span>显示</span>
			<select id="customerLCList" class="combox" name="numPerPage" onchange="navTabPageBreak({numPerPage:this.value})">
